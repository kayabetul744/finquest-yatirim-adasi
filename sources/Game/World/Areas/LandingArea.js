...
export class LandingArea extends Area
{
    ...
    setLetters()
    {
        ...
        for(const reference of references)
        {
            ...
            // Hide the "BRUNO SIMON" letter sculpture (rebranded to Yatırım Adası)
            if(object.visual)
            {
                const worldPosition = new THREE.Vector3()
                object.visual.object3D.getWorldPosition(worldPosition)
                averagePosition.add(worldPosition)
        ...
        averagePosition.divideScalar(references.length)
        this.setWelcomeSign(averagePosition)
    }
    ...
    setWelcomeSign(position)
    {
        ...
        // TextCanvas sets flipY = false to match specially-authored model UVs elsewhere;
        // this mesh uses a plain PlaneGeometry with standard UVs, so flip it back to read correctly
        textCanvas.texture.flipY = true
        ...
        this.game.scene.add(meshBack)
        this.objects.hideable.push(meshBack)
    }
