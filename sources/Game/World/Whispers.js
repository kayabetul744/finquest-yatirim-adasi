...
export class Whispers
{
    ...
    setMenu()
    {
        // The "Leave a whisper" menu tab was removed, so there's no compose UI to wire up anymore.
        // Keep a stub so the ambient (view existing whispers) features below still work.
        if(!this.game.menu.items.get('whispers'))
        {
            this.menu = { inputFlag: { isOpen: false, close: () => {}, countries: new Map() } }
            return
        }
        ...
        this.menu.input.addEventListener('input', () =>
        {
            ...
            this.menu.previewMessageText.textContent = sanatized.length ? sanatized : 'Mesajın burada'
        ...
        {
            const sanatized = sanatize(this.menu.input.value, true, true, true)
            this.menu.previewMessageText.textContent = sanatized !== '' ? sanatized : 'Mesajın burada'
            updateGroup()
        })
        ...
        {
            this.menu.previewMessageText.textContent = 'Mesajın burada'
